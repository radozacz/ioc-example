...
const ICO_REPOSITORY_KEY = 'StoreRepository';
const ACCESS_TOKEN_GENERATOR_KEY = 'AccessTokenGenerator';
...
describe('commonElements', () => {
  ...
  container.register(IOC_TOKEN_KEY, (c) => c.get(ACCESS_TOKEN_GENERATOR_KEY)(c.get(IOC_USERNAME_KEY), c.get(IOC_PASSWORD_KEY)));
  ...
  test('Check store access token generator', () => {
    const tokenGenerator = container.get(ACCESS_TOKEN_GENERATOR_KEY);
